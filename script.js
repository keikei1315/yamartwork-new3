...
const firstRow = document.querySelectorAll('.secondAnimationFirstRowPic'),
      ...
      firstRowMobile = document.querySelectorAll('.secondAnimationFirstRowPicMobile'),
      secondRowMobile = document.querySelectorAll('.secondAnimationSecondRowPicMobile');
    ...
    setTimeout(function(){
      if(navigator.userAgent.match(/(iPhone|iPad|iPod|Android)/i)){
        picAllRight[picAllRight.length - 1].addEventListener('animationend',function(){
          ...
          document.querySelector('.secondAnimationAreaMobile').style.display='block';
          for (let i = 0; i < firstRowMobile.length; i++){
            firstRowMobile[i].classList.add('open');
            firstRowMobile[i].style.animationDelay = (i*.2)+'s';
        }
          for (let i = 0; i < secondRowMobile.length; i++){
            secondRowMobile[i].classList.add('open');
            secondRowMobile[i].style.animationDelay = (i*.2)+'s';
            count =+1;
            console.log(count)
          }
        })
        ...
        picAllRight[picAllRight.length - 1].addEventListener('animationend',function(){
        document.querySelector('.firstAnimation').style.display='none';
        document.querySelector('.secondAnimationPc').style.display='block';
...
secondRow[secondRow.length-1].addEventListener('animationend',function(){
  document.querySelector('.secondAnimation').style.opacity=0;
...
document.querySelector('.topPictureLeftInnerPic').addEventListener('animationend',function(){
    if(window.matchMedia("(max-width:800px)").matches){
      ...
      document.getElementById('backGroundTitle2').style.opacity=1;
      document.querySelector('.scrollArea').style.opacity=1;
      $('.triangleInnerLeftMobile').css({
        borderBottom:31.5+'vh solid transparent',
        borderTop:31.5+'vh solid rgb(49, 49, 49)',
        borderRight:99+'vw solid transparent'
      })
      $('.triangleInnerInnerLeftMobile').css({
        borderBottom:31+'vh solid transparent',
        borderTop:31+'vh solid rgb(255, 241, 211)',
        borderRight:98+'vw solid transparent'
      })
...
secondRowMobile[secondRowMobile.length-1].addEventListener('animationend',function(){
  document.querySelector('.secondAnimationMobile').style.opacity=0;
  document.querySelector('.mainMask').classList.add('open')
...
document.querySelector('.topPictureLeftInnerPic').addEventListener('animationend',function(){
    if(window.matchMedia("(max-width:800px)").matches){
      document.querySelector('.secondAnimationAreaMobile').style.display='none';
      document.getElementById('backGroundTitle').style.opacity=1;
      document.getElementById('backGroundTitle2').style.opacity=1;
      document.querySelector('.scrollArea').style.opacity=1;
      $('.triangleInnerLeftMobile').css({
...
//     $('#galleryTitleArea').on('mousemove',function(e){
//       // $('<div>').classList.add('wave');
//       // $('#galleryTitleArea').prepend($('<div class="wave">'));
